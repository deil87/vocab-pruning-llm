...
\begin{enumerate}[leftmargin=*, itemsep=0pt, topsep=2pt]
  ...
  \item \textbf{Scale evaluation.} The full router suite (excluding cosine)
        re-evaluated on \code{Llama-3.1-8B-Instruct} on two T4 GPUs
        (\code{device\_map="auto"}), quantifying how overhead and quality
...
\caption{Token acceptance rate (\%) vs.\ shortlist size $K$ --- 8B model (T4$\times$2).
         Cosine router not evaluated on 8B due to multi-GPU device
         incompatibility\textsuperscript{$\ddagger$}; Attention Graph omitted
         (prohibitive overhead on T4$\times$2).}
...
Cosine                & \multicolumn{5}{c}{N/A\textsuperscript{$\ddagger$}} \\
Dual-Encoder (step)   & 99.8 & 99.8 & 99.8 & 99.8 & 99.8 \\
DE (prefetch+refresh) & ---  & ---  & ---  & 87.9 & ---  \\
MLP Graph             & 99.8 & 99.8 & 99.8 & 99.8 & 99.8 \\
\bottomrule
\multicolumn{6}{l}{\footnotesize $\ddagger$ Cosine PPL evaluation failed
  silently on T4$\times$2 (\code{lm\_head} on \code{cuda:1}); no result written.}
\end{tabular}
\end{table*}
...
remains near-random. Cosine was not evaluated on 8B due to a multi-GPU
device compatibility issue (footnote $\ddagger$).
...
\caption{Perplexity degradation ($\Delta$PPL) vs.\ shortlist size $K$ --- 8B model (baseline PPL = 16.63).
         ...
         Cosine not evaluated (N/A\textsuperscript{$\ddagger$}).}
\label{tab:ppl-8b}
\small
\begin{tabular}{@{}lrrrrr@{}}
\toprule
...
\multicolumn{6}{l}{\footnotesize $\dagger$ Effective $K{\approx}4{,}112$ (8B).
  \quad $\ddagger$ See Table~\ref{tab:acceptance-8b} footnote.}
\end{tabular}
\end{table*}
...
On the 8B model, quality improves significantly at the same $K$: DE (step)
...
$-16\%$ on 1B), and Static Top-K costs only $-1\%$ to $-2\%$. The DE
...
both \code{Llama-3.2-1B-Instruct} (NVIDIA T4, single GPU) and
\code{Llama-3.1-8B-Instruct} (two NVIDIA T4 GPUs, \code{device\_map="auto"}),
using WikiText-2.
